...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, filterNum, drop_rate):
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_input_features, filterNumList, drop_rate):
        super(_DenseBlock, self).__init__()
        filterIn = num_input_features
        for i, filterNum in enumerate(filterNumList):
            layer = _DenseLayer(filterIn, filterNum, drop_rate)
            filterIn = layer.filterOutNum
            self.add_module('denselayer%d' % (i + 1), layer)
        self.filterOutNum = filterIn  # + filterNum
...
class DenseNet(nn.Module):
    ...
    def __init__(self, in_channels, filternum_conv1=32, drop_rate=0, hidden_size=512):
        super(DenseNet, self).__init__()
        self.featuresMain1 = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv3d(in_channels, filternum_conv1, kernel_size=3, stride=(1, 2, 2), padding=(1, 1, 1), bias=False))]))
        self.hidden_size = hidden_size
        # denseblock encoder
        block1 = _DenseBlock(num_input_features=filternum_conv1, filterNumList=[48, 48], drop_rate=drop_rate)
        self.featuresMain1.add_module('denseblock1', block1)
        num_filters = block1.filterOutNum  # += sum([24, 24])
        trans1 = _Transition(num_filters, 48)
        ...
        block2 = _DenseBlock(num_input_features=48, filterNumList=[64, 64], drop_rate=drop_rate)
        self.featuresMain1.add_module('denseblock2', block2)
        num_filters = block2.filterOutNum
        trans2 = _Transition(num_filters, 64)
        ...
        block3 = _DenseBlock(num_input_features=64, filterNumList=[128, 128], drop_rate=drop_rate)
        self.featuresMain1.add_module('denseblock3', block3)
        num_filters = block3.filterOutNum
        trans3 = _Transition(num_filters, 128)
        ...
        block4 = _DenseBlock(num_input_features=128, filterNumList=[256, 256], drop_rate=drop_rate)
        self.featuresMain1.add_module('denseblock4', block4)

    def forward(self, x):
        x_features = []
        for i in range(len(self.featuresMain1) - 1):
            x = self.featuresMain1[i](x)
            if isinstance(self.featuresMain1[i + 1], _DenseBlock):
                x_features.append(x)
        x = self.featuresMain1[-1](x)
...
class DenseNet3DTower(nn.Module):
    def __init__(self, config):
        ...
        self.vision_tower = DenseNet(in_channels=self.config.image_channel)
...
def build_vision_tower(config, **kwargs):
    ...
    elif 'densenet3d' in vision_tower.lower():
        return DenseNet3DTower(config, **kwargs)
    elif 'swin3d' in vision_tower.lower():
        return Swin3DTower(config, **kwargs)
    else:
...
class LamedLlamaForCausalLM(LamedMetaForCausalLM, LlamaForCausalLM):
    ...
    def generate(
        ...
        **kwargs,
        ...
        position_ids = kwargs["position_ids"] if "position_ids" in kwargs.keys() else None
        attention_mask = kwargs["attention_mask"] if "attention_mask" in kwargs.keys() else None
